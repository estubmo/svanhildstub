 ...
 */
const getMedusaHeaders = (tags: Array<string> = []) => {
  const headers = {
    next: {
      tags,
    },
...
export async function getProductsList({
  ...
  response: { products: Array<ProductPreviewType>; count: number };
...
export async function getHomepageProducts({
  ...
  collectionHandles?: Array<string>;
  currencyCode: string;
  countryCode: string;
}) {
  const collectionProductsMap = new Map<string, Array<ProductPreviewType>>();
...
export async function getCollectionsList(
  offset: number = 0,
  limit: number = 100,
): Promise<{ collections: Array<ProductCollection>; count: number }> {
...
export async function getCategoriesList(
  ...
  product_categories: Array<ProductCategoryWithChildren>;
  count: number;
}> {
...
export async function getCategoryByHandle(
  ...
  const product_categories = [] as Array<ProductCategoryWithChildren>;
